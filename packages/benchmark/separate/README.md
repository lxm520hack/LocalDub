...
|---|---|---|---|---|---|---|
| **无分离 (原始音频)** | — | — | **0** | 8.08% | — | 原始视频 |
| **ORT bag** (vocals-only) | cpu | 1 | **1.397** | **7.54%** | — |  |
| **ORT bag** (4-stems) | cpu | 1 | 6.290 | 7.54% | — | 全导出 |
| **GGML** | cpu | 1 | 1.362 | 8.80% | 4.85% | — |
| **GGML + sidechain** | cpu/vk | 1 | **1.362**† | **7.00%** | — | 🏆 **分离最高质** |
| **PyTorch** | cpu | 1 | 2.647 | 7.72% | 4.49% | — |
| **PyTorch** | cpu | 3 | 7.922 | 7.18% | **3.41%** | CER 最低 |

† 分离 RTF（与 GGML 同，sidechain 仅在 ASR 阶段增加 ffmpeg filter 开销，可忽略）。whisper.cpp Vulkan 转录 RTF **0.081**。

> **推荐：不分离 (原始视频, CER 8.08%) → GGML + sidechain (CER **7.00%**, chars 557 完美匹配 GT)** — sidechain compression 在空白段填入降幅背景音，消除 Demucs 分离低噪引起的 "我好想" 幻觉，同时不破坏有声段。whisper.cpp Vulkan (RTF **0.081**) 进一步降低分离+转录耗时。
...
`compute/asr_seg_compare.ts` 将各后端 ASR 结果逐段与人工标注 (`packages/benchmark/asr_manual.json`) 按时间对齐，统计：
