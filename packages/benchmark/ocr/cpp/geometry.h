...
static Polygon offsetPolygon(const Polygon& poly, float distance) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // Inward normals (rotate 90° CW for clockwise winding)
    ...
    // For outward offset, push vertices in direction of outward normals
    // boxPoints generates CCW order; right normals point outward for CCW
