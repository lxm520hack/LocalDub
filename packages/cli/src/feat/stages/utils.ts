...
export async function updateStageDB(taskId: string, name: string, fields: Record<string, unknown>) {
  if (Object.keys(fields).length === 0) return;
  await db
    .update(taskStages)
    .set(fields)
...
export function readTaskLanguages(sessionPath: string): { asrLanguage: string; targetLanguage: TargetLang } {
